...
- 外观（视觉）优先参考 `shadcn-ui`：使用 `shadcn-ui` 的样式变量、className 与变体（variants）来决定 USS 类名、颜色、圆角、间距与阴影。
...
- 文件位置约定：新建组件放 `Assets/DummyUI/Components/<ComponentName>/`，UXML 放 `Assets/DummyUI/Components/<ComponentName>/<ComponentName>.uxml`，USS 放同目录 `<ComponentName>.uss`。
